...
<body class="flex items-center justify-center min-h-screen bg-gray-100">
    <div class="w-full max-w-md p-6 rounded-lg shadow-md z-10 relative form-container">
        <h2 class="text-2xl font-semibold text-center text-white">Inscription</h2>
        <form action="/signup" method="POST" class="mt-4">
            @csrf
            <div>
                <label class="block text-sm font-medium text-white">Nom</label>
                <input type="text" name="name" required class="w-full p-2 mt-1 rounded-lg input-field">
            </div>
            <div class="mt-2">
            ...
            <div class="mt-2">
                ...
                <input type="password" name="password_confirmation" required class="w-full p-2 mt-1 rounded-lg input-field">
            </div>
            <button type="submit" class="w-full mt-4 p-2 text-white rounded-lg" style="background-color:rgba(225, 206, 206, 0.81);">S'inscrire</button>
        </form>

        <p class="mt-3 text-center text-sm text-white">Déjà un compte ? <a href="/login" class="text-orange-400">Se connecter</a></p>
    </div>
